/*
 * Offline cache for Dragonflies of Singapore.
 *
 * Two caches:
 *  - the app shell (HTML/JS/CSS/icons), precached on install and replaced on
 *    every release (CACHE_VERSION is bumped by admin/build.py);
 *  - the species photos, in a long-lived cache that survives releases. Photos
 *    are cached as they are viewed, and the page asks the worker to fill in the
 *    rest gradually once it is idle ('warm-photos' message), so a new release
 *    never re-downloads ~35 MB and the first visit isn't slowed down.
 * Both URL lists below are regenerated by admin/build.py.
 */

var CACHE_VERSION = 'sgdragonfly-shell-v76';
var PHOTO_CACHE = 'sgdragonfly-photos-v1';
...
var PRECACHE_URLS = [
  ...
  './images/anatomy-dragonfly-v2.jpg',
  './images/app_logo.png',
...
var photoSet = new Set(PHOTO_URLS.map(toAbsolute));
...
self.addEventListener('activate', function (event) {
  event.waitUntil(
    caches.keys().then(function (keys) {
      return Promise.all(
        keys
          .filter(function (key) { return key !== CACHE_VERSION && key !== PHOTO_CACHE; })
          .map(function (key) { return caches.delete(key); })
      );
    }).then(prunePhotos).then(function () {
      return self.clients.claim();
    })
  );
});

// Drop cached photos that are no longer used by any species.
function prunePhotos() {
  return caches.open(PHOTO_CACHE).then(function (cache) {
    return cache.keys().then(function (requests) {
      return Promise.all(requests
        .filter(function (req) { return !photoSet.has(req.url); })
        .map(function (req) { return cache.delete(req); }));
    });
  });
}

// Download the photos that aren't cached yet, two at a time so the app itself
// stays responsive. Safe to interrupt: the next visit carries on where it stopped.
function warmPhotos() {
  return caches.open(PHOTO_CACHE).then(function (cache) {
    return cache.keys().then(function (requests) {
      var have = new Set(requests.map(function (req) { return req.url; }));
      var todo = Array.from(photoSet).filter(function (url) { return !have.has(url); });
...
self.addEventListener('message', function (event) {
  if (event.data === 'warm-photos') {
    event.waitUntil(warmPhotos());
  }
});
...
self.addEventListener('fetch', function (event) {
  ...
  var cacheName = photoSet.has(url.origin + url.pathname) ? PHOTO_CACHE : CACHE_VERSION;
